...
export function RegisterScreen({navigation}: any) {
  ...
  const RegisterUser = async () => {
    try {
      const response = await axios.get('http://192.168.0.44:3000/user/get');
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topArea}>
        ...
        <View style={styles.TextArea}>
          <Text style={styles.Text}>사용자 아이디: {userInfo.id}</Text>
          <Text style={styles.Text}>사용자 이름: {userInfo.name}</Text>
        </View>
      </View>
